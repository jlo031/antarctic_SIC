...
# Path to the feature folder containing directories
...
for dir in "$FEAT_FOLDER_PATH"/*/; do
    ...
    if ! (grep -qx "$DIR_NAME".SAFE "$IMAGE_LIST_FILE_1" || grep -qx "$DIR_NAME".SAFE "$IMAGE_LIST_FILE_2"); then
...
for file in "$RGB_FOLDER_PATH"/*; do
    ...
    if [[ "$file" == *_rgb.json || "$file" == *_rgb.tif ]]; then
        ...
        if ! (grep -qx "$BASE_NAME".SAFE "$IMAGE_LIST_FILE_1" || grep -qx "$BASE_NAME".SAFE "$IMAGE_LIST_FILE_2"); then

            echo "File '$file' is not in either list. Deleting..."
            rm -f "$file" # Delete the file

        else
            echo "File '$file' is in one of the lists. Keeping it."
        fi
